...
CarpoolBridgeの開発環境構築・ビルド・テスト・デプロイ手順をまとめる。
アプリの概要・目的は[README.md](README.md)を参照。
...
新しい利用者（配車担当・管理者）がアプリを使えるようにするには、管理者がFirebase Consoleで
`staffUsers`コレクションへ手動登録する必要がある。認証・権限管理の設計思想は
[docs/06_認証・権限管理設計.md](docs/06_認証・権限管理設計.md)を参照。

### 承認手順

1. 利用者がアプリへ初回アクセスすると、「利用申請」画面にUIDが表示される
2. 利用者はUIDをコピーし、LINE等で管理者へ送信する
3. 管理者はFirebase Console → 対象プロジェクト → Firestore Database を開く
4. `staffUsers`コレクションを開く（未作成の場合は「コレクションを開始」から作成する）
5. 「ドキュメントを追加」し、ドキュメントIDに送られてきたUIDをそのまま入力する
6. フィールドは追加不要（空のドキュメントでよい）。Security Rulesはドキュメントの
   存在有無のみで判定するため、中身は使わない
7. 保存すると、利用者は次回アクセス時からホーム画面を利用できるようになる

### 承認の取り消し

`staffUsers`コレクションから該当UIDのドキュメントをFirestore Consoleで削除する。

### 補足

- MVPでは名前・メールアドレス・ロールなどは保持しない設計としている（docs/06参照）
- 利用者が増えてFirebase Console経由の手動登録が運用に合わなくなった場合は、
  承認画面・pendingUsers方式への拡張を検討する（docs/06参照）
