 ...
 * ref: docs/07_配車アルゴリズム.md#2.2 家族グループ（Family Group）の形成
 *
 * 同一家庭内で乗客ごとに集合場所が異なる場合（一時参加者が家庭と別の集合場所を指定した場合など。
 * 05_データ設計.md#10 type: "temporary"参照）、グループの集合場所には最初に追加された乗客の値を
 * 用いる（スコアリング用の代表値の扱い）。表示用の経由地・集合場所ラベルはCarpool.members側で
 * 乗客ごとに個別解決するため、この代表値による影響は受けない。
 *
...
/**
 * ドライバー家族グループ（車を出す家庭自身の、同乗必須の家族グループ。家庭に
 * 参加コーチがいる場合はそのコーチ自身も含む）の必要席数が対応車両の有効定員を
 * 超過していないか検証したうえで、
