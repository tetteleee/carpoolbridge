...
interface FamilyMemberResponseRowProps {
  /** 対象家族ID（DOM要素のid付与に使用） */
 ...
 * 家族ごとの参加（3状態）・行き／帰りの送迎要否（ミニスイッチ）。
 * 選手（PlayerResponseRow）・コーチ（CoachResponseRow）と全く同じ構造・操作方法とする。
 * 値は呼び出し側（FamilyResponseCard）が保持し、変更の都度Firestoreへ自動保存される。
 */
export function FamilyMemberResponseRow({
  familyMemberId,
  isParticipating,
  noOutwardRide,
  noReturnRide,
  ...
  return (
    <div
      id={`family-member-response-frame-${familyMemberId}`}
      ...
      <div style={rowStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={segmentTrackStyle}>
            <button
              id={`family-member-participating-no-${familyMemberId}`}
              type="button"
            ...
            <button
              id={`family-member-participating-yes-${familyMemberId}`}
              type="button"
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            <RideSwitch
              id={`family-member-no-outward-ride-${familyMemberId}`}
              label="行"
              ariaLabel="行きの送迎"
            ...
            />
            <RideSwitch
              id={`family-member-no-return-ride-${familyMemberId}`}
              label="帰"
              ariaLabel="帰りの送迎"
              noRide={noReturnRide}
              disabled={rideSwitchDisabled}
